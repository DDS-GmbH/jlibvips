...
class VipsImageSpec extends Specification {
    ...
    def "loading vectorised PDFs from memory buffer using fast method"() {
        given: "a large vectorised PDF loaded into memory"
        def pdfFile = copyResourceToFS(pdfResource)
        def pdfBytes = Files.readAllBytes(pdfFile)

        when: "loading it as a vips image from buffer using the fast method"
        def image = VipsImage.fromPdfBufferFast(pdfBytes, pageNumber)
